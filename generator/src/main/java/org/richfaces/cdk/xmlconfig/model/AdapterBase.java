...
public abstract class AdapterBase<Bean, Model> extends XmlAdapter<Bean, Model> {
	...
	/**
	 * <p class="changed_added_4_0">
	 * This method creates adapter object and copies properties from model object to
	 * adapter.
	 * </p>
	 *
	 * @param beanClass
	...
	protected <D, E extends ConfigExtension> E createExtension(D destination)
		...
		return ((Class<E>) method.getReturnType()).getConstructor().newInstance();
	}

	private void copyExtensions(Extensible<ConfigExtension> source, Extensible<ConfigExtension> destination,
			Boolean fromModel) {
		try {
	...
	protected Model createModelElement(Class<? extends Model> modelClass, Bean adapter) {
		try {
			...
			Model modelBean = modelClass.getConstructor().newInstance();
