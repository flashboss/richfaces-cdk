...
public class RendererClassVisitor implements TemplateVisitor {
	...
	private void initializeJavaClass() {
		...
		if (1 == resourceDependencies.size()) {
			...
			for (ResourceDependency resource : resourceDependencies) {
				...
				resources.append("name=\"").append(resource.getName()).append("\",library=\"")
						.append(resource.getLibrary()).append("\",target=\"").append(resource.getTarget()).append("\"")
						.append(")");
			}
			resources.append("}");
			this.generatedClass.addAnnotation(
					new JavaAnnotation(typesFactory.getType(ResourceDependencies.class), resources.toString()));
	...
	private JavaAnnotation createResourceAnnotation(ELType dependencyType, ResourceDependency resource) {
		return new JavaAnnotation(dependencyType, "name=\"" + resource.getName() + "\"",
				"library=\"" + resource.getLibrary() + "\"", "target=\"" + resource.getTarget() + "\"");
	}
	...
	private void flushToMethod(String methodName, boolean enforce, boolean override,
			Collection<Argument> additionaArguments) {
	...
	private void flushToEncodeMethod(String encodeMethodName, boolean enforce) {
		flushToMethod(encodeMethodName, enforce, true, Collections.<Argument>emptyList());
		createMethodContext();
	}
	 ...
	 *
	 * @see
	 * org.richfaces.cdk.templatecompiler.model.TemplateVisitor#visitElement(java.
	 * lang.String)
	 */
